...
import {
  ...
  getAdminCategoriesAPI, reuseAdminCategoryImageAPI, updateAdminCategoryAPI, uploadAdminCategoryImageAPI,
} from "api/admin";
...
function AdminCategoriesPage() {
  ...
  const reset = () => { setEditing(null); setForm(emptyForm); setImage(null); setSelectedMedia(null); if (imageInput.current) imageInput.current.value = ""; };
  const edit = (category) => { setEditing(category); setForm({ name: category.name || "", description: category.description || "", sort_order: category.sort_order || 0, is_active: category.is_active !== false }); setImage(null); setSelectedMedia(null); if (imageInput.current) imageInput.current.value = ""; };
  ...
  const save = async (event) => {
    ...
    try {
      ...
      else if (selectedMedia) await reuseAdminCategoryImageAPI(saved.id, selectedMedia);
      ...
      if (saved) {
        ...
        setImage(null); setSelectedMedia(null);
        await load();
      }
  ...
  const selectImage = (event) => {
    ...
    if (file && !isValidAdminImage(file)) {
      setImage(null); event.target.value = ""; setError(t("admin.products.invalidFiles")); return;
    }
    setImage(file); setSelectedMedia(null);
  };
  const selectExistingImage = (media) => {
    setSelectedMedia(media); setImage(null); setError("");
  ...
  return <main className="admin-page"><div className="container"><div className="admin-page__header"><div><h1 className="admin-page__title">{t("admin.categories.title")}</h1><p className="admin-page__subtitle">{t("admin.categories.subtitle")}</p></div></div>
    ...
    <div className="admin-page__grid"><section className="admin-page__panel"><h2 className="admin-page__panel-title">{t(editing ? "admin.categories.updateTitle" : "admin.categories.createTitle")}</h2><form className="admin-page__form" onSubmit={save}><label>{t("admin.categories.name")}<input name="name" value={form.name} onChange={change} required/></label><label>{t("admin.categories.description")}<textarea name="description" value={form.description} onChange={change}/></label><label>{t("admin.categories.sortOrder")}<input type="number" min="0" name="sort_order" value={form.sort_order} onChange={change}/></label><label className="admin-page__checkbox"><input type="checkbox" name="is_active" checked={form.is_active} onChange={change}/>{t("admin.categories.active")}</label><label>{t("admin.categories.image")}<input ref={imageInput} type="file" accept={ADMIN_IMAGE_ACCEPT} onChange={selectImage}/></label><MediaPicker onSelect={selectExistingImage} disabled={busy}/>{image && <figure className="admin-page__file-preview"><img className="admin-page__preview" src={imagePreview} alt={image.name}/><figcaption><strong>{image.name}</strong><span>{image.type.replace("image/", "").toUpperCase()} · {formatAdminImageSize(image.size)}</span></figcaption></figure>}{selectedMedia && <div><p className="admin-page__selection-label">{t("admin.media.selected")}</p><img className="admin-page__preview" src={selectedMedia.url} alt={selectedMedia.label}/></div>}{!image && !selectedMedia && editing?.image_url && <div><img className="admin-page__preview" src={editing.image_url} alt={editing.name}/>{canDelete && <button type="button" className="admin-page__button admin-page__button--danger" onClick={removeImage} disabled={busy}>{t("admin.categories.removeImage")}</button>}</div>}<div className="admin-page__actions"><button className="admin-page__button" disabled={busy}>{t(editing ? "admin.common.saveChanges" : "admin.categories.createButton")}</button>{editing && <button type="button" className="admin-page__button admin-page__button--ghost" onClick={reset} disabled={busy}>{t("admin.common.cancel")}</button>}</div></form></section>
